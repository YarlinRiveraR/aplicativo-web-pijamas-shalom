...
class Clientes extends Controller
{
    ...
    //NEW!!!
    // Procesa la solicitud de recuperación (se envía vía AJAX desde el modal)
    public function sendRecovery() {
        if (isset($_POST['email']) && !empty($_POST['email'])) {
            $correo = $_POST['email'];
            // Verificamos que el cliente exista (usando getVerificar)
            $cliente = $this->model->getVerificar($correo);
            if (!empty($cliente)) {
                // Generamos un token nuevo para recuperación (no reutilizamos el de confirmación)
